...
# ---------- Early exit if done flag ----------
if (Test-Path -Path $AppsAreInstalled -PathType Leaf) {
    Write-Host "AppsAreInstalled1.0.txt exists. Script will exit."
    return
}



Write-Host "Done1.0.txt or AppsAreInstalled1.0.txt not found."
...
$checkTimer.Add_Tick({
    ...
    if ($count -eq 0) {
        ...
        $sub.Text   = "This may take a while... Do not turn off or restart your computer."
